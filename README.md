...
```bash
pip install autotd-buaa==0.1.13
```
...
从 `0.1.7` 开始，AutoTD 默认开启 telemetry，用于向维护者的 Cloudflare Worker + D1 后台上报安装、运行、停止、用户变化、TD 次数变化和后台运行跨日快照。`0.1.13` 默认 endpoint 为 `https://autotd-telemetry.autotd-buaa.workers.dev`。
...
可启动本地 Web 控制台进行可视化管理：

```bash
autotd ui
```

默认地址为 `http://127.0.0.1:8787`，可通过参数调整：

```bash
autotd ui --host 0.0.0.0 --port 9000
```
